...
class SaliconData(data.Dataset):
    ...
    def __init__(self, root_dir, mean_image_name, section, img_size=(96, 128)):
        ...
        # Resize the mean_image to the correct size
        self.mean_image = cv2.resize(
            self.mean_image, (self.img_size[1], self.img_size[0])
        )
        # Convert to [0, 1] (float)
        self.mean_image = self.mean_image.astype(np.float32) / 255.0
        ...
        if os.name == "posix":
            ...
            with os.scandir(os.path.join(self.root_dir, "images")) as file_iterator:
                file_names = [
                    file_object.name
                    for file_object in list(file_iterator)
                    if section in file_object.name
                ]
        self.image_list = sorted(file_names)

    def __getitem__(self, index):
        # Load the image of given index, and its fixation map
        # (if section == Test, return fully black image as fixation map
        # as they do not exist for test images)
        ...
        # Normalize image by subtracting mean_image,
        # convert from [0, 255] (int) to [0, 1] (float),
        # and reorder from [H, W, C] to [C, H, W]
        ...
        if self.section == "test":
            ...
            fix_map_name = (
                os.path.join(
                    ...
                    "fixation maps",
                    self.section,
                    self.image_list[index][:-4],
                )
                + ".png"
            )  # Images are .jpg, fixation maps are .png

            fix_map = imread(fix_map_name)
            fix_map = imresize(fix_map, self.img_size)

            # Normalize fixation map by
            # converting from [0, 255] (int) to [0, 1] (float),
            # and reordering from [H, W, C] to [C, H, W]
            fix_map = fix_map.astype(np.float32) / 255.0
...
class VideoData(data.Dataset):
    """ A dataset of a single video, loaded from frame files and resized as specified.
    ...
    def __init__(
        ...
        self.mean_image = (
            self.mean_image.astype(np.float32) / 255.0
        )  # Convert to [0, 1] (float)
        ...
        if os.name == "posix":
            ...
            # Filter the list of frames so we only get image files
            # (assumed to be .jpg or .png)
            frame_list = [
                name
            ...
            with os.scandir(os.path.join(self.video_folder, "frames")) as frame_list:
                frame_list = [
                    frame.name
                    for frame in list(frame_list)
                    if frame.name.endswith(".jpg") or frame.name.endswith(".png")
                ]
...
def get_SALICON_datasets(root_dir, mean_image_name, img_size=(480, 640)):
    ...
    mean_image = cv2.resize(
        mean_image, (img_size[1], img_size[0])
    )  # Resize the mean_image to the correct size
...
def get_video_datasets(
    ...
    img_size=(480, 640),
    shuffle=True,
    loader_settings={"batch_size": 2, "num_workers": 8, "pin_memory": False},
):

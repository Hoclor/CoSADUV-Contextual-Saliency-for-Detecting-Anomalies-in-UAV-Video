...
def NSS_loss(x, y):
    """
        ...
        """
    # Check if dimensionality of x is 3.
    # If so, apply NSS loss on each individual set in the batch, and return avg loss
    if len(x.shape) == 2:
        return -NSS(x, y)
    else:
        batch_sum = 0
...
def PCCLoss_torch(x, y):
    ...
    # since cc is in [-1, 1], and 0 is 'bad' and close to -1 or 1 is 'good',
    # return the abs value of cc
    cc = abs(cc)
    # actually return 1 -  cc, as we need to return a loss
    # (since 1 is good,we return loss as 1 - cc)
    loss = 1 - cc

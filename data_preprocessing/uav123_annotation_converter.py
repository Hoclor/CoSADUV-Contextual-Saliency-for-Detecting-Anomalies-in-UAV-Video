...
def draw_annotations(dataset_folder, sequence_name, target_folder=None, img_size=(640, 480), display=False):
...
def draw_groundtruth(dataset_folder, sequence_name, target_folder=None, img_size=(640, 480), display=False):
    tqdm.write(sequence_name)
    # Get the sequnce folder and annotations folder
    sequence_folder = os.path.join(dataset_folder, 'data_seq', 'UAV123', sequence_name)
    annotation_file = os.path.join(dataset_folder, 'anno', 'UAV123', sequence_name + '.txt')
...
def prepare_for_nvvl(dataset_folder, sequence_name, target_folder=None, img_size=(640, 480), display=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--default', dest='default', help='Ignores all other settings and uses default setup', action='store_true', required = False, default=False)
    parser.add_argument('--function', '-f', dest='drawing_function', help='Function to use: \'bounding_boxes\', \'groundtruth\', or \'nvvl\'', required = False)
    parser.add_argument('--width', '-w', dest='width', help='Width of the output frames', required=False, default='640')
    parser.add_argument('--height', '-hh', dest='height', help='Height of the output frames', required=False, default='480')
    ...
    parser.add_argument('--target', '-t', dest='target_folder', help='Name of folder to write results to', required=False, default='C:\\Users\\simon\\GitRepositories\\MastersProject\\DSCLRCN-PyTorch\\Dataset\\UAV123\\train')
    ...
    if args.drawing_function == 'bounding_boxes':
        ...
        print("Error: either --function/-f must be given, or --default must be specified. See -h for more info.")
        quit()
    ...
    if args.default == True:
        ...
        img_size = (640, 480)
        all_sequences = [
            'bike1', 'bike2', 'bike3', 'boat1', 'boat2', 'boat3', 'boat4', 'boat5',
            'boat6', 'boat7', 'boat8', 'boat9', 'building1', 'building2', 'building3',
            'building4', 'building5', 'car10', 'car11', 'car12', 'car13', 'car14',
            'car15', 'car17', 'car18', 'car2', 'car3', 'car4', 'car5', 'car7', 'car9',
            'person1', 'person10', 'person11', 'person13', 'person15', 'person16',
            'person18', 'person20', 'person21', 'person22', 'person23', 'person3',
            'person6', 'person9', 'truck1', 'truck2', 'truck3', 'uav2', 'uav3', 'uav4',
            'uav5', 'uav6', 'uav7', 'uav8', 'wakeboard1', 'wakeboard10', 'wakeboard2',
            'wakeboard3', 'wakeboard4', 'wakeboard5', 'wakeboard6', 'wakeboard7',
            'wakeboard8', 'wakeboard9'
        ]
        ...
        for sequence in tqdm(sequences):
            prepare_for_nvvl(dataset, sequence[0], targets[sequence[1]], img_size, False)
    elif args.name == None:
        ...
        for seq_name in tqdm(sequences):
            drawing_function(args.dataset, seq_name, target_folder=args.target_folder, img_size=img_size, display=args.verbose)
    elif len(args.name.strip().split(',')) > 1:
        for seq_name in tqdm(args.name.strip().split(',')):
            drawing_function(args.dataset, seq_name.strip(), target_folder=args.target_folder, img_size=img_size, display=args.verbose)
    else:
        # Draw bounding boxes on the original video, or ground-truth saliency maps, depending on if -bb was specified
        drawing_function(args.dataset, args.name, target_folder=args.target_folder, img_size=img_size, display=args.verbose)
...
def draw_annotations(
    dataset_folder, sequence_name, target_folder=None, display=False, settings={}
):
...
def prepare_for_nvvl(
    ...
    """ Creates a video of the original sequence, and a separate video of the
    ground truth data, stored in target_folder and target_folder/targets respectively.
    """
    ...
    # If random_start is True, randomly generate a starting frame
    # (that is at least 'duration' frames before the end of the video)
    if random_start:
        try:
    ...
    out_target = cv2.VideoWriter(
        os.path.join(target_folder, "targets", sequence_name + ".avi"),
        fourcc,
        framerate,
        (width, height),
        0,
    ...
    last_time = time.time()
    for frame_count, annotation in enumerate(
        tqdm(annotations[start_time : start_time + duration])
    ):
        # Copy a new blank frame
        frame = np.copy(blank_frame)
...
if __name__ == "__main__":
    ...
    settings = {"random_start": rand_start, "duration": duration}
